...
export default function Notifications({ navigation, route }) {
    ...
    const NotificationDetail = (value) => {
        if (value.title == "Lead Assign") {
            navigation.navigate(navigationStrings.Lead_ManagerDetail, { item: value })
        }
    ...
    const renderItem = ({ item }) => {
        return (
            <TouchableOpacity style={styles.notify} onPress={() => NotificationDetail(item)}>
                ...
                <View>
                    <Text style={{ fontSize: 11, color: '#0F0F0F' }}>{moment(item.updated_at).format("HH:MM A")}</Text>
                </View>
            </TouchableOpacity>
        )
    }
